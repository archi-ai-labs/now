...
const DICT = {
  vi: {
    ...
    'pet.how.rest.p': (o) =>
      `Cái đồng hồ ấy đếm quãng lặng của CLAUDE CODE, không đếm bạn. Mỗi lượt quét hỏi phiên còn thức gần nhất im bao lâu rồi; im từ ${o.m} phút trở lên là mốc nghỉ được đặt lại về bây giờ và thanh tập trung đầy lại — kể cả khi bạn không rời ghế một giây nào. Nên ngồi cả buổi tối mà dải thông số vẫn ghi "đã ngồi 10 phút liền" là chuyện thường: đọc một câu trả lời dài, họp, quay sang cái terminal khác — mỗi quãng như thế quá ${o.m} phút là một lần đồng hồ về 0. Phép đoán này sai hai kiểu NGƯỢC CHIỀU nhau: cắm mặt vào editor cả tiếng mà không gọi Claude thì máy tưởng bạn nghỉ và không nhắc gì; bỏ đi trong lúc một lượt chạy dài đang ghi transcript thì máy tưởng bạn vẫn ngồi và nhắc oan. Thứ không dính cả hai là QUÃNG NGHỈ KHAI TRƯỚC ở công viên: ở đấy ý định do bạn nói ra chứ không do máy đoán, và nó kiểm bằng lượt GÕ CỦA BẠN chứ không bằng lượt ghi của Claude — một lượt chạy dài ghi liên tục, mà đúng quãng ấy mới là quãng bạn rảnh để đứng dậy. Đường thứ ba là caffeine: nó dời mốc nghỉ về trước một phần, hoãn chứ không xoá.`,
    'pet.how.state.t': 'Có những trạng thái nào',
    'pet.how.state.p':
      'Hai thang chạy song song và không thang nào biết thang kia: một thang đọc CÁI BỤNG, một thang đọc SỐ PHÚT NGỒI LIỀN. Dải thông số in cả hai, nên "Ổn · còn 3 giờ nữa thì đói" và "Đang vào nhịp · đã ngồi 10 phút liền" là hai câu về hai chuyện khác nhau chứ không phải một câu nói hai lần. "Ổn" nghĩa là KHÔNG CÓ GÌ ĐỂ NÓI — không phải một lời khen, chỉ là bụng còn trên mốc đói. "Đang vào nhịp" là còn trong pha tỉnh của chu kỳ 90 phút, tức chưa tới lúc phải đứng dậy. Khi cả hai thang cùng có chuyện thì quản gia chỉ VẼ MỘT, theo thứ hạng: việc đang làm thắng tất (đó là thứ bạn vừa bấm, và nó xong trong một phút), rồi tới đói lả, rồi mới tới quá nhịp. Cái hiếm nói trước cái hay xảy ra — một chu kỳ 90 phút quay hết vài lần mỗi ngày, còn đói lả thì hoạ hoằn, mà xếp cái hay nổ lên trên là chôn luôn cái đáng nhìn.',
    'pet.how.state.byFull': 'Nhìn độ no',
    'pet.how.state.bySat': 'Nhìn số phút ngồi liền',
    'pet.how.state.underMin': (o) => `dưới ${o.n} phút`,
    'pet.how.state.rangeMin': (o) => `${o.a}–${o.b} phút`,
    'pet.how.state.overMin': (o) => `quá ${o.n} phút`,
...
7. **Tiền của Cursor là tiền THẬT, tiền của Claude là ước tính.** Số \`$\` của Cursor do chính Cursor tính; số \`$\` của Claude do dashboard nhân ra như ở điểm 1. Hai cột đô này không so với nhau được. Và trần đô của Cursor là số **suy ra** (\`đã tiêu ÷ phần trăm\`), không phải số Cursor gửi — trường \`limit\` trong phản hồi là giá gói, chia theo nó ra 244% trong khi Cursor tự báo 14%. Phần trăm là số gốc.`,
  ...
  en: {
    ...
    'pet.how.rest.p': (o) =>
      `That clock counts silence in CLAUDE CODE, not you. Every scan asks how long the most recently awake session has been quiet; ${o.m} minutes or more and the rest mark is reset to now and focus refills — even if you never left the chair. So sitting all evening while the strip still reads "10 minutes straight" is normal: reading a long answer, a meeting, switching to another terminal — every one of those over ${o.m} minutes resets the clock. The guess fails in two OPPOSITE ways: an hour heads-down in the editor without calling Claude looks like rest, and nothing gets said; walking away while a long run keeps writing the transcript looks like sitting, and you get nagged for a chair you are not in. What avoids both is a DECLARED BREAK in the park: there the intent is stated rather than guessed, and it is checked against YOUR typing, not against Claude writing — a long run writes continuously, and that is exactly the stretch when you are free to stand up. The third road is caffeine: it winds the rest mark partway back, delaying rather than erasing.`,
    'pet.how.state.t': 'The states, and what each word means',
    'pet.how.state.p':
      'Two scales run side by side and neither knows about the other: one reads the STOMACH, one reads the MINUTES SAT. The strip prints both, so "Fine · 3 hours until hungry" and "In the groove · 10 minutes straight" are two statements about two different things, not one thing said twice. "Fine" means NOTHING TO REPORT — not praise, just a stomach above the hunger mark. "In the groove" means still inside the alert phase of a 90-minute cycle, i.e. not yet time to stand up. When both scales have something to say, the butler draws only ONE, by rank: whatever is being done wins outright (it is what you just clicked, and it ends within a minute), then starving, then past-the-cycle. The rare thing outranks the frequent one — a 90-minute cycle turns over several times a day while starving is occasional, and putting the frequent one on top buries the one worth looking at.',
    'pet.how.state.byFull': 'From fullness',
    'pet.how.state.bySat': 'From minutes sat',
    'pet.how.state.underMin': (o) => `under ${o.n} min`,
    'pet.how.state.rangeMin': (o) => `${o.a}–${o.b} min`,
    'pet.how.state.overMin': (o) => `past ${o.n} min`,
